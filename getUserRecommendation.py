...
csv_file_path = './assets/user_blacklist.csv'
...
# Load the spaCy model (large model with word vectors)

nlp = spacy.load("./assets/wine_ner_model")
# Load your wine dataset (replace with your actual dataset)
...
# Sample user query
user_query = "Recommend me a Nouveau from France that tastes of fruits"
user_query = user_query.lower()

#for word in csv_list:
    # Adjust the regex pattern to remove whole words
#    characters_to_remove = r'\b' + re.escape(word) + r'\b'
    # Remove each occurrence of the word from user_query
#    user_query = re.sub(characters_to_remove, '', user_query)
...
if not filtered_wines.empty:
    # Recommend wines based on criteria
    recommendations = filtered_wines[["title", "description", "variety", "country", "designation"]].head(5)
    print("Wine Recommendations:")

    # Print each wine's title, description, and variety separately
